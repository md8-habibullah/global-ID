...
export default function MouseCursor({
  ...
  useEffect(() => {
    ...
    // Ticker Logic (Only runs on desktop)
    const tickerFn = () => {
    ...
    const scrollHandler = () => {
      ...
      if (!isStillOverTarget && currentLeaveHandler) { leaveHandler(); }
    };
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      if (activeTarget && currentLeaveHandler) { target.removeEventListener('mouseleave', currentLeaveHandler); currentLeaveHandler = null; }
      ...
      currentLeaveHandler = () => { leaveHandler(); target.removeEventListener('mouseleave', currentLeaveHandler!); currentLeaveHandler = null; };
      ...
      resumeTimeout = setTimeout(() => {
        if (!activeTarget && cursorRef.current) {
          ...
          gsap.to(cursorRef.current, {
            ...
            ease: 'none',
            onComplete: () => { spinTl.current = gsap.timeline({ repeat: -1 }).to(cursorRef.current!, { rotation: '+=360', duration: spinDuration, ease: 'none' }); }
          });
        }
        resumeTimeout = null;
      }, 50);
  ...
  // If mobile, render NULL (No DOM nodes)
