import type { SVGProps } from "react";

export function DevToIcon(props: SVGProps<SVGSVGElement>) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            aria-label="Dev.to"
            viewBox="0 0 512 512"
            fill="currentColor"
            {...props}
        >
            {/* BACKGROUND: Converted to Outline (Stroke) 
         - Inset (x=24, y=24) prevents the stroke from clipping at the edges.
         - strokeWidth="48" matches the visual weight of Lucide icons.
         - fill="none" makes it transparent inside (wireframe style).
      */}
            <rect
                x="24"
                y="24"
                width="464"
                height="464"
                rx="72"
                fill="none"
                stroke="currentColor"
                strokeWidth="48"
                strokeLinecap="round"
                strokeLinejoin="round"
            />

            {/* TEXT: "DEV"
         - Changed fill to "currentColor" so it takes the parent's text color 
           (gray by default, black/white on hover).
      */}
            <path
                fill="currentColor"
                d="M140.47 203.94h-17.44v104.47h17.45c10.155-.545 17.358-8.669 17.47-17.41v-69.65c-.696-10.364-7.796-17.272-17.48-17.41zm45.73 87.25c0 18.81-11.61 47.31-48.36 47.25h-46.4V172.98h47.38c35.44 0 47.36 28.46 47.37 47.28zm100.68-88.66H233.6v38.42h32.57v29.57H233.6v38.41h53.29v29.57h-62.18c-11.16.29-20.44-8.53-20.72-19.69V193.7c-.27-11.15 8.56-20.41 19.71-20.69h63.19zm103.64 115.29c-13.2 30.75-36.85 24.63-47.44 0l-38.53-144.8h32.57l29.71 113.72 29.57-113.72h32.58z"
            />
        </svg>
    );
}